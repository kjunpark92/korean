...
class GrammarLearning extends React.Component{
  constructor(props){
    super(props);
    this.state = {
      questionNumber: 0,
  ...
  gameMatchingBoxes = () => {
    ...
    if(answersArray[this.state.questionNumber].length < 3){
      ...
      const answers = shuffle(answersArray[this.state.questionNumber].map((answers) => {
        ...
        return(
          <div className="grammarMatchingLearnBox" key={uuid()}>
            <h2>{answers.grammar_point}</h2>
            <h3>{answers.short_explanation}</h3>
            <h4>Examples:</h4>
            <p>{answers.examples}</p>
          </div>
        )
      }));
      return answers;
  ...
  render(){
    const answersArray = this.props.items;
    const gameLength = (answersArray.length) * 4 ;
    ...
    return (
      <div className="mainGameContent">
        <div className="gameTopContent">
          ...
          <div>
            <p>Grammar Points Learned: {this.state.questionNumber * 4}/{gameLength}</p>
          </div>
        </div>
...
export default connect(mapStateToProps)(GrammarLearning);